                    ...
                    <div class="p15">
                        <div class="forum-bloc p15">
                            <h2 class="text-center forum-h2"
                                style="color:#<?= $userForum['color']; ?>"> <?= $slug; ?></h2>
                            <img width="200" class="center-block topic-avatar"
                                 src="<?= $this->Html->url(array('controller' => 'API', 'action' => 'get_head_skin', 'plugin' => false, $slug, 200, 42)); ?>"
                                 alt="Avatar <?= $id; ?>"/>
                            <div class="forum-rank">
                                <?php if (!empty($ranks['rank'])): ?>
                                    <?php foreach ($ranks['rank'] as $key => $rank): ?>
                                        <div <?= $ranks['color'][$key]; ?>
                                                class="forum-badgerank"><?= $rank['group_name']; ?></div>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </div>
                        </div>
                        <?php if (isset($_SESSION['user']) && $_SESSION['user'] != $id): ?>
                            <div class="forum-bloc mt15 p15">
                                <a class="btn-theme btn-full ml0"
                                   href="<?= $this->Html->url('/message/new/' . $slug) ?>"><i
                                            class="fa fa-paper-plane-o"
                                            aria-hidden="true"></i> <?= $Lang->get('FORUM__SEND__MP'); ?></a>
                            </div>
                        <?php endif; ?>
                        <div class="forum-bloc mt15">
                            <div class="forum-bloc-header p15">
                                <h3 class="forum-h3"><i class="fa fa-info-circle"
                                                        aria-hidden="true"></i> <?= $Lang->get('GLOBAL__INFORMATIONS'); ?>
                                </h3>
                                <div class="forum-extrainfo">
                                    <dl>
                                        <dt><?= $Lang->get('FORUM__MSG'); ?><?php if ($infos['nb_message'] > 1) echo 's'; ?>
                                            :
                                        </dt>
                                ...
                                <div class="forum-bloc-header p15">
                                    <h3 class="forum-h3">
                                        <i class="fa fa-share-square-o"
                                           aria-hidden="true"></i> <?= $Lang->get('FORUM__SOCIAL__NETWORK'); ?>
                                        <?php if (isset($_SESSION['user']) && $_SESSION['user'] == $id): ?>
                                            <a class="btn-theme pull-right inline"
                                               href="<?= $this->Html->url('/user/' . $slug . '.' . $id . '/edit') ?>"><i
                                                        class="fa fa-pencil" aria-hidden="true"></i></a>
                                        <?php endif; ?>
                                    </h3>
                                    <div class="text-center">
                                        <?php if (!empty($socialNetworks['facebook'])): ?>
                                            <a class="media media-facebook" target="_blank"
                                               onclick="el('<?= h(strip_tags($socialNetworks['facebook'])); ?>')">
                                        ...
                                        <?php if (!empty($socialNetworks['twitter'])): ?>
                                            <a class="media media-twitter" target="_blank"
                                               onclick="el('<?= h(strip_tags($socialNetworks['twitter'])); ?>')">
                                        <span class="fa-stack fa-lg">
                                              <i class="fa fa-square-o fa-stack-2x"></i>
                                        ...
                                        <?php if (!empty($socialNetworks['youtube'])): ?>
                                            <a class="media media-youtube" target="_blank"
                                               onclick="el('<?= h(strip_tags($socialNetworks['youtube'])); ?>')">
                                        <span class="fa-stack fa-lg">
                                              <i class="fa fa-square-o fa-stack-2x"></i>
                                        ...
                                        <?php if (!empty($socialNetworks['googleplus'])): ?>
                                            <a class="media media-google-plus" target="_blank"
                                               onclick="el('<?= h(strip_tags($socialNetworks['googleplus'])); ?>')">
                                        <span class="fa-stack fa-lg">
                                              <i class="fa fa-square-o fa-stack-2x"></i>
                                        ...
                                        <?php if (!empty($socialNetworks['snapchat'])): ?>
                                            <a class="media media-snapchat" data-toggle="tooltip" data-placement="top"
                                               title="<?= h(strip_tags($socialNetworks['snapchat'])); ?>">
                                        <span class="fa-stack fa-lg">
                                              <i class="fa fa-square-o fa-stack-2x"></i>
                ...
                <div class="col-md-8">
                    <div class="p15">
                        <div class="forum-bloc p15">
                            ...
                            <?php if (isset($_SESSION['user']) && $_SESSION['user'] == $id): ?>
                                <a class="btn-theme pull-right inline"
                                   href="<?= $this->Html->url('/user/' . $slug . '.' . $id . '/edit') ?>"><i
                                            class="fa fa-pencil"
                                            aria-hidden="true"></i> <?= $Lang->get('GLOBAL__EDIT'); ?></a>
                            <?php endif; ?>
                            <div>
                        ...
                        <ul class="nav nav-tabs mt15" role="tablist">
                            <li role="presentation" class="active"><a href="#comment" aria-controls="comment" role="tab"
                                                                      data-toggle="tab"><?= $Lang->get('FORUM__COMMENT'); ?></a>
                            </li>
                            <li role="presentation"><a href="#thumb" aria-controls="profile" role="tab"
                                                       data-toggle="tab"><?= $Lang->get('FORUM__FEEDBACK'); ?></a></li>
                        </ul>
                            ...
                            <div role="tabpanel" class="tab-pane active" id="comment">
                                ...
                                <?php foreach ($lasts['Comment'] as $last): ?>
                                    ...
                                    <div class="forum-profile-comment">
                                        <h4>
                                            <a href="<?= $last['Topic']['href']; ?>"><?= h($last['Topic']['title']); ?></a>
                                        </h4>
                            ...
                            <div role="tabpanel" class="tab-pane" id="thumb">
                                ...
                                <?php foreach ($lasts['Note'] as $last): ?>
                                    ...
                                    <div class="forum-profile-appreciate">
                                        <p>
                                            <i class="fa fa-thumbs-<?= $last['Note']['fa']; ?> thumb-<?= $last['Note']['class']; ?>"
                                               aria-hidden="true"></i> <?= $slug . $last['Note']['txt']; ?><a
                                                    href="<?= $last['Note']['msg']['href']; ?>"><?= $this->Text->truncate(strip_tags($last['Note']['message']), 150); ?></a>
                                        </p>
                                    </div>
                                <?php endforeach; ?>
                            </div>
        ...
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                ...
                <div class="modal-body">
                    <p>Vous allez quitter le site <b><?= $_SERVER['SERVER_NAME']; ?></b> pour vous rendre sur cette url
                        :</p>
